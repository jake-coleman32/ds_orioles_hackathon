setwd("/Users/Jake/Documents/orioles_hackathon")
context <- read.csv("context.csv")
bat_split <- read.csv("Batting_Splits.csv")
bat_stat <- read.csv("Batting_Statistics.csv")
lg_avg <- read.csv("League_Averages.csv")
pitch_split <- read.csv("Pitching_Splits.csv")
pitch_stat <- read.csv("Pitching_Statistics.csv")
pfx <- read.csv("Pitchfx.csv")
...
stuff_dset = as.data.frame(games_r_dset[1:35216,1:3]) %>%
...
pitch_model <- function(){
  for(i in 1:n){
    mu[i] <- alpha[pitcher[i]] + gamma*X1[i] + gamma^2*X2[i] + 
...
parameters = c("alpha","gamma","sd.a", "sd.g")
